            ...
            <div v-if="trip" class="d-flex p-4">
                <div class="col">
                    ...
                    <div class="categories-grid">
                        <div v-for="(category, index) in trip.categories" :key="index" class="category-item"
                            :style="{ borderColor: category.color }">
                            <div class="category-square" :style="{ backgroundColor: category.color }"></div>
                            <label>{{ category.name }}</label>
                            <button v-if="isOrganizer" type="button" class="ml-1 bold bi bi-x-lg bi-sm" style="margin-left:auto" @click="removeCatFromTrip(category)"></button>
                        </div>
                        <!-- Add Category Button -->
                        <div v-if="isOrganizer" class="category-item add-category cursor-pointer" @click="openCategoryModal">
                            ...
                            <label class="cursor-pointer flex-1 text-center mr-5">Add Category</label>
                        </div>
                    </div>
                </div>
                <div class="col2">
                    ...
                    <form class="row g-3 needs-validation mx-5" @submit.prevent="saveChanges">
                        <div class="col-md-12">
                            ...
                            <input v-model="trip.title" :readonly="!editMode" type="text" class="form-control form-control-lg" @input="validateForm"
                                :class="{ 'is-invalid': formSubmitted && titleValid , 'is-invalid': formSubmitted&& !titleValid }"
                                id="title">
                            <div v-if="formSubmitted && !titleValid" class="invalid-format" >{{ errorMessage.title }}</div>
                        </div>
                        <div class="col-md-6">
                            <label for="destination" class="form-label">Destination</label>
                            <input v-model="trip.destination" :readonly="!editMode" type="text" class="form-control form-control-lg" @input="validateForm"
                                :class="{ 'is-invalid': formSubmitted && destinationValid , 'is-invalid': formSubmitted&& !destinationValid }"
                            id="place">
                            <div v-if="formSubmitted && !destinationValid" class="invalid-format" >{{ errorMessage.destination }}</div>
                        </div>
                        <div class="col-md-6">
                            <label for="created_at" class="form-label">Created At</label>
                            <input v-model="trip.created_at" readonly type="text" class="form-control form-control-lg"
                                id="created_at">
                        </div>
                        <div class="col-md-6">
                            <label for="start_date" class="form-label">Start Date</label>
                            <input v-model="trip.start_date" :readonly="!editMode" type="date" class="form-control form-control-lg" @input="validateForm"
                                :class="{ 'is-invalid': formSubmitted && startDateValid , 'is-invalid': formSubmitted&& !startDateValid }"
                                id="start_date" >
                        </div>
                        <div class="col-md-6">
                            <label for="end_date" class="form-label">End Date</label>
                            <input v-model="trip.end_date" :readonly="!editMode" type="date" class="form-control form-control-lg" @input="validateForm"
                                :class="{ 'is-invalid': formSubmitted && endDateValid , 'is-invalid': formSubmitted&& !endDateValid }"
                                id="end_date">
                        </div>
                        <div v-if="formSubmitted && !startDateValid || formSubmitted && !endDateValid" class="invalid-format">{{ errorMessage.startDate }}</div>
                        <div class="col-md-12">
                            <label for="description" class="form-label">Description</label>
                            <textarea v-model="trip.description" :readonly="!editMode" class="form-control" id="description" @input="validateForm"
                                :class="{ 'is-invalid': formSubmitted && descriptionValid , 'is-invalid': formSubmitted&& !descriptionValid }"
                                rows="4"></textarea>
                        </div>
                        <div   v-if="formSubmitted && !descriptionValid" class="invalid-format">{{ errorMessage.description }}</div>
                        <button v-if="!editMode && isOrganizer " class="ml-2 btn btn-primary col-md-2" type="button" @click="editMode = true">Edit</button>
                        <button v-if="editMode && isOrganizer" class="ml-2 btn btn-primary col-md-2"  type="submit">Save</button>
                    </form>
                </div>
            </div>
        </template>
                    ...
                    <div class="modal-body">
                        <label for="category">Category</label>
                            ...
                            <select v-model="selectedCategory" id="category" class="form-select custom-select">
                                ...
                                <option v-for="(category, index) in availableCategories" :key="index" :value="category">{{ category.name }}</option>
                            </select>
...
const validateForm = () => {
    ...
    if (!trip.value.end_date) {
        ...
        if(trip.value.end_date < formatDate(highestEventDate.value.start)){
...
const saveNewCategories = async () => {
    trip.value.categories = [...trip.value.categories, ...selectedCategories.value];

    try {
        const tripDoc = doc(firestore, 'trips', tripId);
        const tripData = {
            categories: trip.value.categories,
        }
        await updateDoc(tripDoc, tripData);
        //getTrip();
        closeCategoryModal();
